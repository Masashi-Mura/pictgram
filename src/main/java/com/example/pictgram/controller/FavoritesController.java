...
public class FavoritesController {
	...
	//お気に入りのみ表示動作
	//favoriteテーブルからログインuserIdのレコードを取得し、List<TopicForm>に格納する
	//動作:1.favoriteテーブルの該当レコードをList<Favorite> topicsに格納
	//     2.topicsからTopic topicを取得→TopicForm formにマッピング→List<TopicForm> listに格納
	//     3.2をレコード数分繰り返す
	//     4.2のTopic→TopicFormマッピングは、TopicsControllerクラスのgetTopicメソッドを使用
	@GetMapping(path = "/favorites")
	public String index(Principal principal, Model model) throws IOException {
		Authentication authentication = (Authentication) principal;
		UserInf user = (UserInf) authentication.getPrincipal();
		//List<Favorite> topicsを、List<TopicForm> listにコピー
	...
	//お気に入り登録の動作(favoriteテーブルにuserIdとtopicIdを記録)
	...
	public String create(Principal principal, @RequestParam("topic_id") long topicId,
		...
		//userIdとtopicIdがfavoriteテーブルに存在しない場合、favoriteテーブルにそのtopicIdとuserIdを登録
        ...
        System.out.println("テストコメント フェバリットコントローラPOST /favorite→redirect:/topics");
		return "redirect:/topics";		
	}

	//お気に入り解除の動作(favoriteテーブルから、userIdとtopicIdに一致するレコードを削除)
	...
	public String destroy(Principal principal, @RequestParam("topic_id") long topicId,
        ...
        System.out.println("テストコメント フェバリットコントローラDELETE /favorite→redirect:/topics");
		return "redirect:/topics";
	}
